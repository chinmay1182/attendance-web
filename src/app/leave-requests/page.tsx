...
export default function LeavesPage() {
    ...
    useEffect(() => {
        ...
        // Realtime Leaves
        const channel = supabase.channel('leaves_realtime_v2')
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'leave_requests' },
                async (payload) => {
                    // For Admin/HR: Always refresh all requests to see employee submissions
                    if (profile?.role === 'admin' || profile?.role === 'hr') {
                        await fetchAllRequests();
                    }

                    // For all users: Refresh my list if the change affects me
    ...
    const fetchPolicies = async () => {
        try {
            ...
            if (res.ok) {
                ...
                console.error('❌ Failed to fetch policies:', res.status, res.statusText);
            }
        } catch (e) {
    ...
    const fetchMyRequests = async () => {
        try {
            ...
            if (data.data) setMyRequests(data.data as any);
        ...
        <>
            ...
            <div className={styles.container}>
                ...
                <div className={styles.tabs}>
                    ...
                    {!isAdminOrHr && <button onClick={() => setActiveTab('my_leaves')} className={`${styles.tabBtn} ${activeTab === 'my_leaves' ? styles.active : ''}`}>My Leaves</button>}
                ...
                {activeTab === 'dashboard' && (
                    <>
                        <div className={styles.statsGrid}>
                            {/* Show employee-perspective stats for all users including admin */}
                        ...
                        {/* Balances Section - Show for all users */}
                        <>
                        ...
                        {/* Recent Activity - Show user's own requests */}
                        <div className={styles.card}>
                            ...
                            <table className={styles.table}>
                                ...
                                <tbody>
                                    ...
                                    {myRequests.length === 0 && <tr><td colSpan={4} style={{ textAlign: 'center', padding: '20px' }}>No recent activity</td></tr>}
                                </tbody>
                            </table>
                        </div>
                    </>
